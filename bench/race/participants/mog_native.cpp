...
int main(int argc, char** argv) {
    ...
    for (int run = 0; run < runs; ++run) {
        ...
        // Single pass preserving stream order; strategy rows fire when the
        // market-row cursor reaches their recorded trigger.
        ...
        // Per-order aggregates for cross-engine agreement (round 3):
        // [ref, side, vwap_price, total_qty]
    ...
    std::printf("mog-native: %.0f ops/s (%zu fills, deterministic=%d)\n", ops / med, fills,
                std::all_of(digests.begin(), digests.end(),
                            [&](const std::string& d) { return d == digests[0]; }));
    return 0;
}
